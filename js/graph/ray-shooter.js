...
export function runRayShooter(rows) {
    ...
    // ════════════════════════════════════════════════════════════════════
    // PASS 1 — Orphaned fittings → EP1 of any non-orphaned candidate
    //   Orphan initiators : Unpaired non-PIPE, non-nonMappable types
    //                       (FLANGE, VALVE, ELBOW, TEE, OLET …)
    //   Candidate pool    : full rows array — filter: !_isOrphan(c)
    // ════════════════════════════════════════════════════════════════════
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const hit = _shootOrphan(orphan, candidatePool, (c) => !_isOrphan(c));
        if (hit) _resolveOrphan(orphan, hit, hit.pass);
    }

    // ════════════════════════════════════════════════════════════════════
    // PASS 1.5 — Orphan-to-Orphan bridging (Deep Ray Mode Construct)
    //   Because native pipes are structurally severed (RaySkip:T), virtually all 
    //   components exist as floating Unpaired topological islands. Pass 1 natively 
    //   fails when isolated nodes seek Paired anchors. Pass 1.5 physically fuses 
    //   Orphan-to-Orphan spatial strings together using the newly integrated 
    //   __hitTargets bidirectional collision tracker to inherently suppress Z-fighting.
    // ════════════════════════════════════════════════════════════════════
    for (let i = 0; i < rows.length; i++) {
        const orphan = rows[i];
        if (!_isEligible(orphan)) continue;
    ...
    const branchCandPool = rows.filter(r => {
        ...
        // Include the branch-face row if its component is "connected":
        //   • the row itself is non-orphan, OR
        //   • any sibling row with the same RefNo is non-orphan (handles Ray Mode reset)
        if (!_isOrphan(r)) return true;
        return r.RefNo ? (_refHasResolved.get(String(r.RefNo)) === true) : false;
    });
