...
function wireToggles() {
  ...
  // Tolerance + Decimals + Max Segment Length
  ["cfg-tolerance", "cfg-decimals", "cfg-maxSegmentLength"].forEach(id => {
    ...
    const cfg = getConfig();
    // Ensure default for maxSegmentLength
...
function renderRayShooterSettings() {
  ...
  container.querySelectorAll("#rs-maxRayLength, #rs-passP3").forEach(inp => inp.addEventListener("change", saveRs));
}
